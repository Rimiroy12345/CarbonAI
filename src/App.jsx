...
export default function App() {
  ...
  // Calculation estimates (in tCO2e)
  const electricityEmissions = (parseFloat(emissions.electricity) || 0) * 0.00085;
  const gasEmissions = (parseFloat(emissions.naturalGas) || 0) * 0.00202;
  const petrolEmissions = (parseFloat(emissions.petrol) || 0) * 0.00231;
  const dieselEmissions = (parseFloat(emissions.diesel) || 0) * 0.00268;
  const flightEmissions = (parseFloat(emissions.flights) || 0) * 0.00015;
  const hotelEmissions = (parseFloat(emissions.hotels) || 0) * 0.03;
  const commuteEmissions = (parseFloat(emissions.commuting) || 0) * 0.00012;
  const wasteEmissions = (parseFloat(emissions.waste) || 0) * 0.0005;

  const energyEmissions = electricityEmissions + gasEmissions;
  const transportEmissions = petrolEmissions + dieselEmissions + flightEmissions + hotelEmissions + commuteEmissions;

  const totalEmissions = energyEmissions + transportEmissions + wasteEmissions;

  const percentage = (val) => {
    if (!totalEmissions || totalEmissions === 0) return 0;
    return Math.round((val / totalEmissions) * 100);
  };
  ...
  const largestSource = Object.entries(emissionBreakdown).reduce(
  ...
  try {
    ...
    if (res && (res.recommendation || res.aiRecommendation)) {
      setAiRecommendation(res.recommendation || res.aiRecommendation);
    }
    setStep(4);
  } catch (err) {
    console.error("Calculation failed:", err);
    //alert("Backend connection timeout or error. Showing standard fallback assessment.");
    setStep(4); // Advances to Step 4 so user is not stuck on error
  } finally {
    setLoading(false);
  }
};
                ...
                <div className="setup-card">
                  ...
                  <div className="setup-actions">
                    ...
                    <button
                     ...
                     >
                    {loading ? 'Analyzing with AI...' : 'Calculate footprint →'}
                    </button>
                  </div>
